...
_RECOMMENDED_GOVULNCHECK = "v1.2.0"
...
def _ensure_govulncheck() -> str:
    ...
    if not exe:
        raise RuntimeError(
            ...
            f"golang.org/x/vuln/cmd/govulncheck@{_RECOMMENDED_GOVULNCHECK} "
            "(see tool-versions.json; pin this tag for reproducible CI)."
        )
    return exe


def _find_go_module_root(start_path: Path) -> Path | None:
...
def compute_reachability_go(
    ...
    with open(vulns_json, encoding="utf-8") as f:
